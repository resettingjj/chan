...
public class Flock : MonoBehaviour
{
    ...
    //NativeArray<Vector2> tempAgentMove; // 추가
    ...
    void Update()
    {
        ...
        //tempAgentMove = new NativeArray<Vector2>(agents.Count, Allocator.TempJob); // 추가
    ...
    void LateUpdate() {
        jobHandle.Complete();
        for (int i = 0; i < agents.Count; i++) {
            //agents[i].Move(tempAgentMove[i]); // 수정
